...
vector<string> checkHorizontal(int x, int y, vector<string> matrix, string currentWord)
{
    ...
    for (int i = 0; i < n; i++) 
    {
        ...
        {                                   // this shows that word cannot
            matrix[0][0] = '@';             // be placed horizontally
            return matrix;                                
        }
    }
    return matrix;
...
// this function checks for the current word
// if it can be placed vertically or not // x ->
// it represent index of row
// y -> it represent index of column
// currentWord -> it represent the
// current word in word array
...
// the index of current word n -> it represent // the
// length of row or column of the square matrix
